...
const SignupController = async (req, res) => {
  try {
    ...
    if (!name || !cnic || !email || !password) {
      return res
        .status(400)
        .send({ status: 400, message: "All inputs Required" });
    }
    ...
    if (userExist) {
      return res
        .status(400)
        .send({ status: 400, message: "user already exist" });
    }
...
const ForgotPasswordController = async (req, res) => {
  try {
    ...
    const sendingForgotPasswordLink = `https://hackthone1-six.vercel.app/resetPassword/${resetToken}`;
...
const ResetPasswordController = async (req, res) => {
  try {
    ...
    if (password !== confirmPassword) {
      return res
        .status(400)
        .send({ status: 400, message: "confirm password not match" });
    }
    ...
    if (!userFound) {
      res
        .status(400)
        .send({ status: 400, message: "Invalid or expired token" });
    }
    ...
    if (now > userFound.resetPasswordTokenExpire) {
      res
        .status(400)
        .send({ status: 400, message: "The reset token has been expired" });
    }
    ...
    res
      .status(201)
      .send({ status: 201, message: "Password Reset successfully" });
  } catch (error) {
    res.status(500).send({ status: 500, message: error.message });
  }
};
